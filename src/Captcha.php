...
class Captcha
{
     ...
     * @return \think\Response
     */
    public function cache_entry($id = '')
    {
        // 图片宽(px)
        $this->imageW || $this->imageW = $this->length * $this->fontSize * 1.5 + $this->length * $this->fontSize / 2;
        // 图片高(px)
        ...
        // 缓存保存验证码
        $random_key = $this->randomCode();
        Cache::set($random_key, $secode, 3600);
        ...
        $content = ob_get_clean();
        imagedestroy($this->im);
        return ['content' => "data:image/png;base64," . base64_encode($content), 'key' => $random_key];
    }
    ...
    {
        if (empty($code) || empty($key)) return false;
        if (Cache::has($key)) {
            $data = Cache::pull($key);
        }else {
            $data = array();
        ...
        // 验证验证码是否已过期
        if (!isset($data['verify_time']) || !isset($data['verify_code'])) return false;
        if (time() > $data['verify_time'] + 1800) return false;
    ...
    private function randomCode() {
        $random_code = md5(uniqid() . rand(10000, 99999) . rand(10, 99));
        if (Cache::has($random_code)) {
            $this->randomCode();
        }
        return $random_code;
    }
